...
class RaydiumSwap {
  ...
  async loadPoolKeys(liquidityFile: string) {
    const data = await readFile('pools.json');
    const pools = JSON.parse(data.toString());

    const liquidityJson = pools as { official: any; unOfficial: any };
